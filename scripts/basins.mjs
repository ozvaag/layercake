...
async function ensure(reg) { const dir = join(ROOT, 'data/sources/hydrobasins'); const shp = join(dir, `hybas_${reg}_lev${LEV}_v1c.shp`); if (existsSync(shp)) return shp; mkdirSync(dir, { recursive: true }); const zip = join(dir, `${reg}_${LEV}.zip`); if (!existsSync(zip)) { console.log(`downloading HydroBASINS ${reg} level ${LEV}…`); const r = await fetch(`https://data.hydrosheds.org/file/HydroBASINS/standard/hybas_${reg}_lev${LEV}_v1c.zip`); if (!r.ok) throw new Error('hydrobasins ' + r.status); await new Promise((ok, no) => { const w = createWriteStream(zip); r.body.pipeTo(new WritableStream({ write(c) { w.write(c); }, close() { w.end(ok); }, abort: no })); }); } execSync(`unzip -qo "${zip}" -d "${dir}"`); return shp; }
