...
async function ensure(reg) { const dir = join(ROOT, 'data/sources/hydrorivers'); const shp = join(dir, `HydroRIVERS_v10_${reg}_shp/HydroRIVERS_v10_${reg}.shp`); if (existsSync(shp)) return shp; mkdirSync(dir, { recursive: true }); const zip = join(dir, `${reg}.zip`); if (!existsSync(zip)) { console.log(`downloading HydroRIVERS ${reg} (60–110 MB)…`); const r = await fetch(`https://data.hydrosheds.org/file/HydroRIVERS/HydroRIVERS_v10_${reg}_shp.zip`, { headers: { 'user-agent': 'Mozilla/5.0 (layercake)' } }); if (!r.ok) throw new Error('hydrorivers ' + r.status); await new Promise((ok, no) => { const w = createWriteStream(zip); r.body.pipeTo(new WritableStream({ write(c) { w.write(c); }, close() { w.end(ok); }, abort: no })); }); } execSync(`unzip -qo "${zip}" -d "${dir}"`); return shp; }
